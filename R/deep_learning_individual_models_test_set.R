...
# field season test sets, based on seperate models trained on images from each
# year. For validation, we used hand-counted total kernel numbers.
...
results_2018 <- import_tensorflow_summary("./data/test_set_two_models_predictions_2018_summary.tsv", hand_counted)
results_2019 <- import_tensorflow_summary("./data/test_set_two_models_predictions_2019_summary.tsv", hand_counted)
females_2019 <- read.table(file = "./data/significant_females.tsv", header = T, sep = '\t')
...
###### Plotting Supplemental Figure 4C ######
### 2019 Fluorescent females
ggplot(females_2019, aes(x = GFP_hand, y = GFP_tf)) +
  geom_abline(slope = 1, intercept = 0, linetype = 'dashed', size = 1, color = '#4D4D4D') +
...
ggsave(filename = './plots/tensorflow_GFP_2019_females.png', 
       device = 'png', 
       width = 5,
       height = 5, 
       dpi = 400,
...
### 2019 Nonfluorescent females
ggplot(females_2019, aes(x = wt_hand, y = wt_tf)) +
  geom_abline(slope = 1, intercept = 0, linetype = 'dashed', size = 1, color = '#4D4D4D') +
...
ggsave(filename = './plots/tensorflow_wt_2019_females.png', 
       device = 'png', 
       width = 5,
       height = 5, 
       dpi = 400,
...
### 2019 Percent GFP females
ggplot(females_2019, aes(x = 100 * percent_GFP_hand, y = 100 * percent_GFP_tf)) +
  geom_abline(slope = 1, intercept = 0, linetype = 'dashed', size = 1, color = '#4D4D4D') +
...
ggsave(filename = './plots/tensorflow_percent_2019_females.png', 
       device = 'png', 
       width = 5,
       height = 5, 
       dpi = 400,
...
mean_total_kernels <- mean(c(results_2018$GFP_hand, results_2019$GFP_hand) +
                             c(results_2018$wt_hand, results_2019$wt_hand))

# Mean absolute deviations 2018
mean_abs_dev_fluorescent_2018 <- mean(abs(results_2018$GFP_hand - results_2018$GFP_tf))
mean_abs_dev_wt_2018 <- mean(abs(results_2018$wt_hand - results_2018$wt_tf))
mean_abs_dev_percent_2018 <- mean(abs(results_2018$percent_GFP_hand - results_2018$percent_GFP_tf))

# Mean absolute deviations 2019
mean_abs_dev_fluorescent_2019 <- mean(abs(results_2019$GFP_hand - results_2019$GFP_tf))
mean_abs_dev_wt_2019 <- mean(abs(results_2019$wt_hand - results_2019$wt_tf))
mean_abs_dev_percent_2019 <- mean(abs(results_2019$percent_GFP_hand - results_2019$percent_GFP_tf))
