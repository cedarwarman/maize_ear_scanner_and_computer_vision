...
# ###### Cropping validation set to match scikit-image prediction dimensions ######
...
# 
# 
# #Running the cropping on the test datset
# filenames <- list.files(path = './data/xml_testing_set_annotations', pattern="*.xml", full.names=TRUE)
# 
...
# write.table(cropped_df, file = "./data/cropped_test_set_counts.txt", sep = "\t", row.names = FALSE, quote = FALSE)
   

###### Importing and processing the data ######
# Note: it takes awhile to crop the xml coordinate files by 15% on either 
# side to match the tranditional approach cv results. To do this process,
# uncomment and run the previous section. Otherwise it is loaded here:
hand_counts_cropped <- read.table(file = "./data/cropped_test_set_counts.txt", 
                                  sep = "\t",
                                  header = TRUE)
colnames(hand_counts_cropped) <- c("image_name", "GFP_hand", "wt_hand")


sk_image_counts <- read.table(file = "./data/sk_kernel_counter_output.txt",
                              sep = "\t",
                              header = FALSE)
